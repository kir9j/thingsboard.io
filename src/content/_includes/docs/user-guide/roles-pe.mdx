...
| **Write** | Create or modify entities, their configuration, and attributes |
...
| **Assign to Tenant** | Assign a device to another tenant |
...
<table>
  ...
  <tr>
    <td><strong>Widgets Bundle</strong></td>
...
A single action often touches more than one resource, so a restricted role usually needs more than the one permission it appears to require. This is the most common source of "silent" permission failures — an action is denied even though the role has the permission that obviously matches.

<Aside type="caution">
A user whose role grants **All** resources / **All** operations — including the default **Tenant Administrator** role — is never affected by these dependencies. They only surface for **custom roles** that grant a narrower set of permissions, such as a read-only role.
</Aside>

A few recurring patterns explain most of them:

- **Entity groups** — to act on the entities inside a group you need the group permission (**Add to Group** / **Remove from Group**) *and* **Read** on each entity.
...
The table below lists each action where ThingsBoard checks a **second** permission before it will succeed.
...
| Configure the **QR code widget** for a bundle | **Read** on the **mobile bundle** |
...
| **Share** an entity group | **Write** on the group, **Write** on the target user group, and **Read** on the **role** |
| **Add** or **remove** entities in a group | **Add to Group** / **Remove from Group** on the group, and **Read** on each entity |
| Create a device, asset, entity view, customer, user, or edge **directly into a group** | **Read** on that **entity group** |
| Change an entity's **owner** | **Change Owner** on the entity; assigning it into target groups also needs **Add to Group** |
| Assign a rule chain, integration, scheduler event, or group **to an edge** | **Write** on the **edge** and **Read** on the assigned resource |
| View an **audit log** for a specific entity, customer, or user | **Read** on that entity, customer, or user |
| Manage **customer-level white labeling** | **Read** on the **customer** |
